...
namespace crnlib
{
   ...
   void crn_threading_init()
   {
      ...
      g_number_of_processors = math::maximum<uint>(1U, g_system_info.dwNumberOfProcessors);
       ...
       size_t len = sizeof(maxprocessors);
       // Note: This gets the number of processors currently availible in the
       // current power mode. You will probably only notice a difference on a
       // laptop or other mobile device with a battery.
       sysctlbyname("hw.physicalcpu", &maxprocessors, &len, NULL, 0);
       
       g_number_of_processors = math::maximum<int>(1, maxprocessors);
   ...
   crn_thread_id_t crn_get_current_thread_id()
   {
      // FIXME: Not portable
#ifdef _MAC
       // This is bullcrap since pthread_self returns an opaque type ~Ryan
   ...
   semaphore::semaphore(long initialCount, long maximumCount, const char* pName)
   {
       ...
       printf("creating semiphore %s, initial count %ld, max count %ld\n", pName, initialCount, maximumCount);
       ...
       m_sem = sem_open(pName, O_CREAT, 0600, 0);
   ...
   bool semaphore::wait(uint32 milliseconds)
   {
      ...
      {
#ifdef _MAC
          // TODO: Timed semaphore is not availible on Darwin
          status = sem_wait(m_sem);
          //CRNLIB_FAIL("semaphore: sem timedwait unavailible on Darwin");
          printf("semaphore: sem timedwait unavailible on Darwin\n");
#else
         struct timespec interval;
         interval.tv_sec = milliseconds / 1000;
         interval.tv_nsec = (milliseconds % 1000) * 1000000L;
   ...
   {
#ifdef _MAC
       if(!OSSpinLockTry(&m_spinlock))
       {
           CRNLIB_FAIL("spinlock: OSSpinLockTry() failed");
       }
#else
