...
namespace DataPacking
{
     ...
     */
    void NetworkUnpackage::UnpackToEntities(CEntityBase *pEntities[],
                                                unsigned int numEntities)
    {
        ASSERTION(m_pDataBuffer, "Trying to unpack NULL network package!");
        
        ASSERTION(m_pDataBuffer->m_iNum_Blocks == numEntities,
                  "Network package incompatible with client entities!");
        
        for(int i = 0; i < numEntities; ++i)
        {
            WriteToEntity(&pEntities[i], i);
        }
    }
     ...
     */
    void NetworkUnpackage::WriteToEntity(CEntityBase **ppBaseClass, unsigned int blockIndex)
    {
