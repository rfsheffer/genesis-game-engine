...
MemoryPool::~MemoryPool()
{
	...
	for(size_t i = 0; i < m_PoolInfo.size(); i++)
	{
		...
		while(curDebugInfo < PoolInfo.m_debugInfo.size())
		{
			...
			{
				char buf[256];
				sprintf_s(buf, "Memory Not Freed: Pool %d, alloc of %d bytes in %s at %d!\n", i + 1, bytes, PoolInfo.m_debugInfo[curDebugInfo].file, PoolInfo.m_debugInfo[curDebugInfo].line);
				OutputDebugString(buf);
			}

			size_t movement = (bytes + sizeof(unsigned int));
...
//-----------------------------------------------------------------------------
unsigned int MemoryPool::CreatePool(const char *pName, size_t size)
{
	...
	// General Setup
	strncpy_s( newPool.m_szPoolName, pName, MAX_POOL_NAME );
...
//-----------------------------------------------------------------------------
void *MemoryPool::MemAlloc(size_t bytes, unsigned int pool, const char *file, int line)
{
	if(bytes == 0 || pool >= m_PoolInfo.size())
		return NULL;

	size_t alloc_size = bytes + sizeof(unsigned int);
	...
	*pAlloc = (bytes << NUM_MEM_INFO_BITS) | MEM_INFO_INUSE_BIT; // Squish in the size and the info bits
	...
	strncpy_s(debuginfo.file, file, MAX_DEBUG_FILE_NAME);
	debuginfo.line = line;
...
bool MemoryPool::FreeAlloc(void *pMem)
{
	...
	*pInfo = *pInfo & ~MEM_INFO_INUSE_BIT;
	...
	size_t size = (*pInfo >> NUM_MEM_INFO_BITS);
...
void MemoryPool::DumpMemory(void)
{
	...
	for(unsigned int i = 0; i < m_PoolInfo.size(); i++)
	{
		...
		fprintf(pFile, "\nMemory Pool %d\n", i);
		for(size_t j = 0; j < m_PoolInfo[i].m_numBytes; j++)
		{
